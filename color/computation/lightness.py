...
def get_luminance_equation(primaries, whitepoint):
    ...
    return "Y = {0}(R) + {1}(G) + {2}(B)".format(
        *numpy.ravel(color.computation.derivation.get_normalized_primary_matrix(primaries, whitepoint))[3:6])
...
def get_luminance(RGB, primaries, whitepoint):
    ...
    X, Y, Z = numpy.ravel(color.computation.derivation.get_normalized_primary_matrix(primaries, whitepoint))[3:6]
