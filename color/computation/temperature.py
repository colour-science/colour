...
def get_planckian_table(uv, cmfs, start, end, count):
    ...
    for Ti in numpy.linspace(start, end, count):
        spd = color.computation.blackbody.blackbody_spectral_power_distribution(Ti, *cmfs.shape)
        XYZ = color.computation.tristimulus.spectral_to_XYZ(spd, cmfs)
        XYZ *= 1. / numpy.max(XYZ)
        UVW = color.computation.transformations.XYZ_to_UCS(XYZ)
        ui, vi = color.computation.transformations.UCS_to_uv(UVW)
...
def CCT_to_uv_ohno(CCT,
    ...
    spd = color.computation.blackbody.blackbody_spectral_power_distribution(CCT, *cmfs.shape)
    XYZ = color.computation.tristimulus.spectral_to_XYZ(spd, cmfs)
    XYZ *= 1. / numpy.max(XYZ)
    UVW = color.computation.transformations.XYZ_to_UCS(XYZ)
    u0, v0 = color.computation.transformations.UCS_to_uv(UVW)

    if Duv == 0.:
        return u0, v0
    else:
        spd = color.computation.blackbody.blackbody_spectral_power_distribution(CCT + delta, *cmfs.shape)
        XYZ = color.computation.tristimulus.spectral_to_XYZ(spd, cmfs)
        XYZ *= 1. / numpy.max(XYZ)
        UVW = color.computation.transformations.XYZ_to_UCS(XYZ)
        u1, v1 = color.computation.transformations.UCS_to_uv(UVW)
...
def D_illuminant_CCT_to_xy(CCT):
    ...
    if 4000 <= CCT <= 7000:
        ...
        raise color.utilities.exceptions.ProgrammingError(
            "Correlated color temperature must be in domain [4000, 25000]!")
