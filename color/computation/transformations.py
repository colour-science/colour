...
def XYZ_to_RGB(XYZ,
    ...
    *chromatic adaptation* method, *normalized primary matrix* and *transfer function*.
    ...
    cat = color.computation.chromatic_adaptation.get_chromatic_adaptation_matrix(
...
def XYZ_to_Luv(XYZ,
    ...
    L = 116. * yr ** (
        1. / 3.) - 16. if yr > color.computation.lightness.CIE_E else color.computation.lightness.CIE_K * yr
...
def Luv_to_XYZ(Luv,
    ...
    Y = ((
             L + 16.) / 116.) ** 3. if L > color.computation.lightness.CIE_E * color.computation.lightness.CIE_K else L / color.computation.lightness.CIE_K
...
def XYZ_to_Lab(XYZ,
    ...
    fx = xr ** (1. / 3.) if xr > color.computation.lightness.CIE_E else (
                                                                             color.computation.lightness.CIE_K * xr + 16.) / 116.
    fy = yr ** (1. / 3.) if yr > color.computation.lightness.CIE_E else (
                                                                             color.computation.lightness.CIE_K * yr + 16.) / 116.
    fz = zr ** (1. / 3.) if zr > color.computation.lightness.CIE_E else (
                                                                             color.computation.lightness.CIE_K * zr + 16.) / 116.
...
def Lab_to_XYZ(Lab,
    ...
    xr = fx ** 3. if fx ** 3. > color.computation.lightness.CIE_E else (
                                                                            116. * fx - 16.) / color.computation.lightness.CIE_K
    yr = ((
              L + 16.) / 116.) ** 3. if L > color.computation.lightness.CIE_K * color.computation.lightness.CIE_E else L / color.computation.lightness.CIE_K
    zr = fz ** 3. if fz ** 3. > color.computation.lightness.CIE_E else (
                                                                            116. * fz - 16.) / color.computation.lightness.CIE_K
