...
class IsIdentityTestCase(unittest.TestCase):
	"""
	Defines :func:`color.matrix.isIdentity` definition units tests methods.
	"""

	def testInterpolateMatrices(self):
		"""
		Tests :func:`color.matrix.isIdentity` definition.
		"""

		self.assertTrue(color.matrix.isIdentity(numpy.matrix([1, 0, 0, 0, 1, 0, 0, 0, 1]).reshape(3, 3)))
		self.assertFalse(color.matrix.isIdentity(numpy.matrix([1, 2, 0, 0, 1, 0, 0, 0, 1]).reshape(3, 3)))
		self.assertTrue(color.matrix.isIdentity(numpy.matrix([1, 0, 0, 1]).reshape(2, 2), n=2))
		self.assertFalse(color.matrix.isIdentity(numpy.matrix([1, 2, 0, 1]).reshape(2, 2), n=2))

class LinearInterpolateMatricesTestCase(unittest.TestCase):
	"""
	Defines :func:`color.matrix.linearInterpolateMatrices` definition units tests methods.
	"""

	def testLinearInterpolateMatrices(self):
		"""
		Tests :func:`color.matrix.linearInterpolateMatrices` definition.
		"""

		numpy.testing.assert_almost_equal(color.matrix.linearInterpolateMatrices(2850,
																				 7500,
																				 COLOR_MATRIX_1,
																				 COLOR_MATRIX_2,
																				 6500),
