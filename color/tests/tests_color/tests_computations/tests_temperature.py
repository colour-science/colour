...
**Description:**
    Defines units tests for :mod:`color.computation.temperature` module.
...
PLANCKIAN_TABLE = [
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1000.0, ui=0.44801089464064786,
                                                        vi=0.35462498085812383, di=0.25378213254223697),
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1001.1111111111111, ui=0.44775082571296082,
                                                        vi=0.35464751834928576, di=0.25352949944454956),
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1002.2222222222222, ui=0.4474910632226583,
                                                        vi=0.35467001024049455, di=0.25327717785676873),
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1003.3333333333334, ui=0.44723160701057407,
                                                        vi=0.35469245651987269, di=0.253025167595974),
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1004.4444444444445, ui=0.44697245691544729,
                                                        vi=0.35471485717585538, di=0.2527734684771403),
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1005.5555555555555, ui=0.44671361277394045,
                                                        vi=0.35473721219718934, di=0.25252208031315576),
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1006.6666666666666, ui=0.44645507442066867,
                                                        vi=0.35475952157292978, di=0.2522710029148514),
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1007.7777777777778, ui=0.446196841688206,
                                                        vi=0.35478178529244025, di=0.2520202360910075),
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1008.8888888888889, ui=0.44593891440711669,
                                                        vi=0.35480400334538981, di=0.2517697796483851),
    color.computation.temperature.PLANCKIAN_TABLE_TUVD(Ti=1010.0, ui=0.44568129240596821,
...
class TestGetPlanckianTable(unittest.TestCase):
    """
    Defines :func:`color.computation.temperature.get_planckian_table` definition units tests methods.
    """
    ...
    def test_get_planckian_table(self):
        ...
        numpy.testing.assert_almost_equal(
            map(to_list, color.computation.temperature.get_planckian_table((0.1978, 0.3122), cmfs, 1000, 1010, 10)),
            map(to_list, PLANCKIAN_TABLE))


class TestGetPlanckianTableMinimalDistanceIndex(unittest.TestCase):
    """
    Defines :func:`color.computation.temperature.get_planckian_table_minimal_distance_index` definition units tests methods.
    """
...
class Testuv_to_CCT_ohno(unittest.TestCase):
    """
    Defines :func:`color.computation.temperature.uv_to_CCT_ohno` definition units tests methods.
    """
    ...
    def test_uv_to_CCT_ohno(self):
        ...
        numpy.testing.assert_almost_equal(color.computation.temperature.uv_to_CCT_ohno((0.1978, 0.3122), cmfs),
                                          ...
                                          decimal=7)
        numpy.testing.assert_almost_equal(color.computation.temperature.uv_to_CCT_ohno((0.4328, 0.2883), cmfs),
                                          (1041.8672179878763, -0.067377582642145384),
                                          decimal=7)
        numpy.testing.assert_almost_equal(
            color.computation.temperature.uv_to_CCT_ohno((0.2927, 0.2722), cmfs, iterations=4),
...
class TestCCT_to_uv_ohno(unittest.TestCase):
    """
    Defines :func:`color.computation.temperature.CCT_to_uv_ohno` definition units tests methods.
    """
    ...
    def test_CCT_to_uv_ohno(self):
        ...
        numpy.testing.assert_almost_equal(
            ...
            decimal=7)
        numpy.testing.assert_almost_equal(
            color.computation.temperature.CCT_to_uv_ohno(1041.849524611546, -0.067377582728534946, cmfs),
            (0.43280250331413772, 0.28829975758516474),
            decimal=7)
        numpy.testing.assert_almost_equal(
            color.computation.temperature.CCT_to_uv_ohno(2448.9489053326438, -0.084324704634692743, cmfs),
...
class Testuv_to_CCT_robertson(unittest.TestCase):
    """
    Defines :func:`color.computation.temperature.uv_to_CCT_robertson` definition units tests methods.
    """

    def test_uv_to_CCT_robertson(self):
        """
        Tests :func:`color.computation.temperature.uv_to_CCT_robertson` definition.
        """

        for key, value in TEMPERATURE_DUV_TO_UV.iteritems():
            numpy.testing.assert_almost_equal(color.computation.temperature.uv_to_CCT_robertson(value), key, decimal=0)


class TestCCT_to_uv_robertson(unittest.TestCase):
    """
    Defines :func:`color.computation.temperature.CCT_to_uv_robertson` definition units tests methods.
    """

    def test_CCT_to_uv_robertson(self):
        """
        Tests :func:`color.computation.temperature.CCT_to_uv_robertson` definition.
        """

        for i in range(2000, 49501, 2500):
            for j in numpy.arange(-0.05, 0.075, 0.025):
                numpy.testing.assert_almost_equal(color.computation.temperature.CCT_to_uv_robertson(i, j),
...
class TestD_illuminant_CCT_to_xy(unittest.TestCase):
    """
    Defines :func:`color.computation.temperature.D_illuminant_CCT_to_xy` definition units tests methods.
    """

    def test_D_illuminant_CCT_to_xy(self):
        """
        Tests :func:`color.computation.temperature.D_illuminant_CCT_to_xy` definition.
        """

        numpy.testing.assert_almost_equal(color.computation.temperature.D_illuminant_CCT_to_xy(4000),
                                          (0.38234362499999996, 0.3837662610155782),
                                          decimal=7)
        numpy.testing.assert_almost_equal(color.computation.temperature.D_illuminant_CCT_to_xy(7000),
                                          (0.3053574314868805, 0.3216463454745523),
                                          decimal=7)
        numpy.testing.assert_almost_equal(color.computation.temperature.D_illuminant_CCT_to_xy(25000),
