...
class TestWavelength_to_XYZ(unittest.TestCase):
    ...
    def test_wavelength_to_XYZ(self):
        """
        Tests :func:`color.transformations.wavelength_to_XYZ` definition.
        """

        numpy.testing.assert_almost_equal(
            color.transformations.wavelength_to_XYZ(480,
        ...
        numpy.testing.assert_almost_equal(
            color.transformations.wavelength_to_XYZ(641.5,
                                                    color.spectral.STANDARD_OBSERVERS_COLOR_MATCHING_FUNCTIONS.get(
                                                        "Standard CIE 2006 2 Degree Observer")),
...
class TestSpectral_to_XYZ(unittest.TestCase):
    ...
    def test_spectral_to_XYZ(self):
        """
        Tests :func:`color.transformations.spectral_to_XYZ` definition.
        """

        cmfs = color.spectral.STANDARD_OBSERVERS_COLOR_MATCHING_FUNCTIONS.get("Standard CIE 1931 2 Degree Observer")
        numpy.testing.assert_almost_equal(
            color.transformations.spectral_to_XYZ(RELATIVE_SPD_DATA.resparse(*cmfs.shape),
                                                  cmfs,
                                                  color.spectral.ILLUMINANTS_RELATIVE_SPD.get("A").resparse(
                                                      *cmfs.shape)),
            numpy.matrix([0.14463719, 0.10858326, 0.02046648]).reshape((3, 1)),
            decimal=7)
        ...
        numpy.testing.assert_almost_equal(
            color.transformations.spectral_to_XYZ(RELATIVE_SPD_DATA.resparse(*cmfs.shape),
                                                  ...
                                                  color.spectral.ILLUMINANTS_RELATIVE_SPD.get("C").resparse(
                                                      *cmfs.shape)),
            numpy.matrix([0.10770425, 0.09448703, 0.06627423]).reshape((3, 1)),
            decimal=7)
        ...
        numpy.testing.assert_almost_equal(
            color.transformations.spectral_to_XYZ(RELATIVE_SPD_DATA.resparse(*cmfs.shape),
                                                  color.spectral.STANDARD_OBSERVERS_COLOR_MATCHING_FUNCTIONS.get(
                                                      "Standard CIE 1964 10 Degree Observer"),
                                                  color.spectral.ILLUMINANTS_RELATIVE_SPD.get("F2").resparse(
                                                      *cmfs.shape)),
...
class TestRGB_to_CMY(unittest.TestCase):
    """
    Defines :func:`color.transformations.RGB_to_CMY` definition units tests methods.
    """

    def test_RGB_to_CMY(self):
        """
        Tests :func:`color.transformations.RGB_to_CMY` definition.
        """

        numpy.testing.assert_almost_equal(
            color.transformations.RGB_to_CMY(numpy.matrix([0.25, 0.60, 0.05]).reshape((3, 1))),
            numpy.matrix([0.75, 0.40, 0.95]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            color.transformations.RGB_to_CMY(numpy.matrix([0., 0., 0.]).reshape((3, 1))),
            numpy.matrix([1., 1., 1.]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            color.transformations.RGB_to_CMY(numpy.matrix([1., 1., 1.]).reshape((3, 1))),
            numpy.matrix([0., 0., 0.]).reshape((3, 1)),
            decimal=7)
...
class TestCMY_to_RGB(unittest.TestCase):
    """
    Defines :func:`color.transformations.CMY_to_RGB` definition units tests methods.
    """

    def test_CMY_to_RGB(self):
        """
        Tests :func:`color.transformations.CMY_to_RGB` definition.
        """

        numpy.testing.assert_almost_equal(
            color.transformations.CMY_to_RGB(numpy.matrix([0.75, 0.40, 0.95]).reshape((3, 1))),
            numpy.matrix([0.25, 0.60, 0.05]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            color.transformations.CMY_to_RGB(numpy.matrix([1., 1., 1.]).reshape((3, 1))),
            numpy.matrix([0., 0., 0.]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            color.transformations.CMY_to_RGB(numpy.matrix([0., 0., 0.]).reshape((3, 1))),
            numpy.matrix([1., 1., 1.]).reshape((3, 1)),
            decimal=7)
...
class TestCMY_to_CMYK(unittest.TestCase):
    """
    Defines :func:`color.transformations.CMY_to_CMYK` definition units tests methods.
    """

    def test_CMY_to_CMYK(self):
        """
        Tests :func:`color.transformations.CMY_to_CMYK` definition.
        """

        numpy.testing.assert_almost_equal(
            color.transformations.CMY_to_CMYK(numpy.matrix([0.75, 0.40, 0.95]).reshape((3, 1))),
            numpy.matrix([0.58333333, 0., 0.91666667, 0.4]).reshape((4, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            color.transformations.CMY_to_CMYK(numpy.matrix([0.15, 1., 1.]).reshape((3, 1))),
            numpy.matrix([0., 1., 1., 0.15]).reshape((4, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            color.transformations.CMY_to_CMYK(numpy.matrix([0.15, 0., 0.]).reshape((3, 1))),
...
class TestCMYK_to_CMY(unittest.TestCase):
    """
    Defines :func:`color.transformations.CMYK_to_CMY` definition units tests methods.
    """

    def test_CMYK_to_CMY(self):
        """
        Tests :func:`color.transformations.CMYK_to_CMY` definition.
        """

        numpy.testing.assert_almost_equal(
            color.transformations.CMYK_to_CMY(numpy.matrix([0.58333333, 0., 0.91666667, 0.4]).reshape((4, 1))),
            numpy.matrix([0.75, 0.40, 0.95]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            color.transformations.CMYK_to_CMY(numpy.matrix([0., 1., 1., 0.15]).reshape((4, 1))),
            numpy.matrix([0.15, 1., 1.]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            color.transformations.CMYK_to_CMY(numpy.matrix([0.15, 0., 0., 0.]).reshape((4, 1))),
