...
class TestLuminousFlux(unittest.TestCase):
    ...
    def test_luminous_flux(self):
        ...
        self.assertAlmostEqual(
            luminous_flux(ILLUMINANTS_SPDS['F2'].copy().normalise()),
            28588.73612977,
            places=7)

        self.assertAlmostEqual(
            luminous_flux(LIGHT_SOURCES_SPDS['Neodimium Incandescent']),
            23807.65552737,
            places=7)

        self.assertAlmostEqual(
            luminous_flux(LIGHT_SOURCES_SPDS['F32T8/TL841 (Triphosphor)']),
            13090.06759053,
            places=7)
...
class TestLuminousEfficiency(unittest.TestCase):
    ...
    def test_luminous_efficiency(self):
        ...
        self.assertAlmostEqual(
            luminous_efficiency(ILLUMINANTS_SPDS['F2'].copy().normalise()),
            0.49317624,
            places=7)

        self.assertAlmostEqual(
            luminous_efficiency(LIGHT_SOURCES_SPDS['Neodimium Incandescent']),
            0.19943936,
            places=7)

        self.assertAlmostEqual(
            luminous_efficiency(
                LIGHT_SOURCES_SPDS['F32T8/TL841 (Triphosphor)']),
            0.51080919,
            places=7)
...
class TestLuminousEfficacy(unittest.TestCase):
    ...
    def test_luminous_efficacy(self):
        ...
        self.assertAlmostEqual(
            luminous_efficacy(ILLUMINANTS_SPDS['F2'].copy().normalise()),
            336.83937176,
            places=7)

        self.assertAlmostEqual(
            luminous_efficacy(LIGHT_SOURCES_SPDS['Neodimium Incandescent']),
            136.21708032,
            places=7)

        self.assertAlmostEqual(
            luminous_efficacy(LIGHT_SOURCES_SPDS['F32T8/TL841 (Triphosphor)']),
            348.88267549,
            places=7)
