...
class LUT3D(AbstractLUT):
    ...
    def invert(self,
        ...
        table = LUT_t.table
        LUT_t.table = LUT.apply(LUT_t.table, interpolator, interpolator_kwargs)
        ...
        query = tree.query(table, query_size, n_jobs=-1)[-1]
        if query_size == 1:
            LUT_q.table = table.reshape(-1, 3)[query].reshape(
                [target_size, target_size, target_size, 3])
        else:
            LUT_q.table = np.mean(
                table.reshape(-1, 3)[query],
