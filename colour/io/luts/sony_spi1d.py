...
def read_LUT_SonySPI1D(path):
    ...
    domain = np.array([domain_min, domain_max])

    if dimensions == 1:
        return LUT1D(np.squeeze(table), title, domain, comments=comments)
    elif dimensions == 2:
        return LUT2D(table, title, np.tile(domain, (3, 1)), comments=comments)
...
def write_LUT_SonySPI1D(LUT, path, decimals=7):
    ...
    >>> domain = np.array([[-0.1, 1.5], [-0.1, 1.5], [-1.0, 1.5]])
    >>> LUT = LUT2D(
