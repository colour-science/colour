...
LUTS_DIRECTORY: str = os.path.join(os.path.dirname(__file__), "resources", "cinespace")
...
class TestReadLUTCinespace(unittest.TestCase):
    ...
    def test_read_LUT_Cinespace(self):
        ...
        LUT_4 = read_LUT_Cinespace(os.path.join(LUTS_DIRECTORY, "Explicit_Domain.csp"))
...
class TestWriteLUTCinespace(unittest.TestCase):
    ...
    def test_write_LUT_Cinespace(self):
        ...
        LUT_4_r = read_LUT_Cinespace(os.path.join(LUTS_DIRECTORY, "Ragged_Domain.csp"))
        ...
        LUT_5_r = read_LUT_Cinespace(
            os.path.join(LUTS_DIRECTORY, "Three_Dimensional_Table_With_Shaper.csp")
        )
        LUT_5_r.sequence[0] = LUT_5_r.sequence[0].as_LUT(LUT1D, force_conversion=True)
        write_LUT_Cinespace(
            LUT_5_r,
            os.path.join(
        ...
        LUT_6_r.sequence[0] = LUT_6_r.sequence[0].as_LUT(LUT3x1D, force_conversion=True)
        write_LUT_Cinespace(
            LUT_6_r,
            os.path.join(
                self._temporary_directory,
