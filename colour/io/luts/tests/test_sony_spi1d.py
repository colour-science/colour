...
ROOT_LUTS: str = os.path.join(os.path.dirname(__file__), "resources", "sony_spi1d")


class TestReadLUTSonySPI1D:
    """
    ...
    def test_read_LUT_SonySPI1D(self) -> None:
        ...
        assert LUT_1.name == "eotf sRGB 1D"
        assert LUT_1.dimensions == 1
        xp_assert_equal(LUT_1.domain, [-0.1, 1.5])
        assert LUT_1.size == 16
        assert LUT_1.comments == [
            'Generated by "Colour 0.3.11".',
            '"colour.models.eotf_sRGB".',
        ]

        LUT_2 = read_LUT_SonySPI1D(os.path.join(ROOT_LUTS, "eotf_sRGB_3x1D.spi1d"))
        assert LUT_2.comments == [
            'Generated by "Colour 0.3.11".',
            '"colour.models.eotf_sRGB".',
        ]
        xp_assert_equal(LUT_2.domain, [[-0.1, -0.1, -0.1], [1.5, 1.5, 1.5]])


class TestWriteLUTSonySPI1D:
    """
    ...
    def test_write_LUT_SonySPI1D(self) -> None:
        ...
        LUT_2_r = read_LUT_SonySPI1D(os.path.join(ROOT_LUTS, "eotf_sRGB_3x1D.spi1d"))
        write_LUT_SonySPI1D(
            LUT_2_r,
