...
class TestIES_TM2714_Sd(unittest.TestCase):
    ...
    def test_read(self, sd: Optional[SpectralDistribution] = None):
        ...
        test_read: List[Tuple[Dict, Union[
            Header_IESTM2714, SpectralDistribution_IESTM2714]]] = [
                (FLUORESCENT_FILE_HEADER,
                 sd.header), (FLUORESCENT_FILE_SPECTRAL_DESCRIPTION, sd)
            ]
    ...
    def test_write(self):
        ...
        for attribute in ('manufacturer', 'catalog_number', 'description',
                          'document_creator', 'unique_identifier',
                          'measurement_equipment', 'laboratory',
                          'report_number', 'report_date',
                          'document_creation_date', 'comments'):
            self.assertEquals(
                getattr(sd_r.header, attribute), getattr(
                    sd_t.header, attribute))

        for attribute in ('spectral_quantity', 'reflection_geometry',
                          'transmission_geometry', 'bandwidth_FWHM',
                          'bandwidth_corrected'):
            self.assertEquals(
