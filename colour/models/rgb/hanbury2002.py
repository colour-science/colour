...
def RGB_to_IHLS(RGB):
    ...
    References
    ----------
    :cite: A_3D-Polar_Coordinate_Colour_Representation_Suitable_for_Image_Analysis
    https://www.researchgate.net/publication/243602454_A_3D-Polar_Coordinate_Colour_Representation_Suitable_for_Image_Analysis
    ...
    H2 = np.where(H1 < 0, 0.00,
                  H1)  # condition of H1 if its any values smaller than 0 if so that value will be converted to zero
    H3 = np.where(H2 < 60, H2,
                  0)  # condition of H1 if any value larger than 60 if so that value will be converted to zero
...
def IHLS_to_RGB(IHLS):
    ...
    H2 = np.where(H1 < 0, 0.00,
                  H1)  # condition of H1 if its any values smaller than 0 if so that value will be converted to zero
    H3 = np.where(H2 < 60, H2,
                  0.00)  # condition of H1 if any value larger than 60 if so that value will be converted to zero
    H4 = np.max(H3)  # #extraction of the correct range of H1 from array of zero
