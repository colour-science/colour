...
log_encoding_DJIDLog` definition.
        ...
        xp_assert_close(
            log_encoding_DJIDLog(xp_as_array(0.0, xp=xp)),
            0.0929,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_DJIDLog(xp_as_array(0.18, xp=xp)),
            0.398764556189331,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_DJIDLog(xp_as_array(1.0, xp=xp)),
            0.584555,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_DJIDLog` definition n-dimensional arrays support.
        ...
        x = 0.18
        y = as_ndarray(log_encoding_DJIDLog(xp_as_array(x, xp=xp)))
...
log_encoding_DJIDLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_DJIDLog(xp_as_array(x * factor, xp=xp)),
                    y * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_DJIDLog` definition.
        ...
        xp_assert_close(
            log_decoding_DJIDLog(xp_as_array(0.0929, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_DJIDLog(xp_as_array(0.398764556189331, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS * 10,
        )

        xp_assert_close(
            log_decoding_DJIDLog(xp_as_array(0.584555, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS * 10,
        )
...
log_decoding_DJIDLog` definition n-dimensional arrays support.
        ...
        y = 0.398764556189331
        x = as_ndarray(log_decoding_DJIDLog(xp_as_array(y, xp=xp)))
...
log_decoding_DJIDLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_DJIDLog(xp_as_array(y * factor, xp=xp)),
                    x * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
