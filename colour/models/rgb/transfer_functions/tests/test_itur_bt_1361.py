...
class TestOetf_BT1361:
    ...
    def test_oetf_BT1361(self, xp: ModuleType) -> None:
        """
...
oetf_BT1361` definition.
        ...
        xp_assert_close(
            oetf_BT1361(xp_as_array(-0.18, xp=xp)),
            -0.212243985492969,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_BT1361(xp_as_array(0.0, xp=xp)), 0.0, atol=TOLERANCE_ABSOLUTE_TESTS
        )

        xp_assert_close(
            oetf_BT1361(xp_as_array(0.015, xp=xp)),
            0.067500000000000,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            oetf_BT1361(xp_as_array(1.0, xp=xp)), 1.0, atol=TOLERANCE_ABSOLUTE_TESTS
        )

    def test_n_dimensional_oetf_BT1361(self, xp: ModuleType) -> None:
        """
        Test :func:`colour.models.rgb.transfer_functions.itur_bt_1361.\
oetf_BT1361` definition n-dimensional arrays support.
        ...
        L = 0.18
        V = as_ndarray(oetf_BT1361(xp_as_array(L, xp=xp)))

        L = xp.tile(xp_as_array(L, xp=xp), (6,))
        V = xp.tile(xp_as_array(V, xp=xp), (6,))
        xp_assert_close(oetf_BT1361(L), V, atol=TOLERANCE_ABSOLUTE_TESTS)
    ...
    def test_domain_range_scale_oetf_BT1361(self, xp: ModuleType) -> None:
        """
        Test :func:`colour.models.rgb.transfer_functions.itur_bt_1361.\
oetf_BT1361` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_BT1361(xp_as_array(L * factor, xp=xp)),
                    V * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
oetf_inverse_BT1361` definition unit tests methods.
    ...
    def test_oetf_inverse_BT1361(self, xp: ModuleType) -> None:
        """
        Test :func:`colour.models.rgb.transfer_functions.itur_bt_1361.\
oetf_inverse_BT1361` definition.
        ...
        xp_assert_close(
            oetf_inverse_BT1361(xp_as_array(-0.212243985492969, xp=xp)),
            -0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT1361(xp_as_array(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT1361(xp_as_array(0.067500000000000, xp=xp)),
            0.015,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT1361(xp_as_array(0.409007728864150, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT1361(xp_as_array(1.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

    def test_n_dimensional_oetf_inverse_BT1361(self, xp: ModuleType) -> None:
        """
        Test :func:`colour.models.rgb.transfer_functions.itur_bt_1361.\
oetf_inverse_BT1361` definition n-dimensional arrays support.
        ...
        V = 0.409007728864150
        L = as_ndarray(oetf_inverse_BT1361(xp_as_array(V, xp=xp)))

        V = xp.tile(xp_as_array(V, xp=xp), (6,))
        L = xp.tile(xp_as_array(L, xp=xp), (6,))
        xp_assert_close(oetf_inverse_BT1361(V), L, atol=TOLERANCE_ABSOLUTE_TESTS)
    ...
    def test_domain_range_scale_oetf_inverse_BT1361(self, xp: ModuleType) -> None:
        """
        Test :func:`colour.models.rgb.transfer_functions.itur_bt_1361.\
oetf_inverse_BT1361` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_inverse_BT1361(xp_as_array(V * factor, xp=xp)),
                    L * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
