...
class TestXYZ_to_Luv(unittest.TestCase):
    ...
    def test_XYZ_to_Luv(self):
        ...
        np.testing.assert_almost_equal(
            colour.models.cie_luv.XYZ_to_Luv(np.array([0.96907232, 1., 1.12179215])),
            np.array([100., -11.27488915, -29.36041662]).reshape((3, 1)),
            decimal=7)

        np.testing.assert_almost_equal(
            colour.models.cie_luv.XYZ_to_Luv(np.array([1.92001986, 1., -0.1241347])),
            np.array([100., 331.44911128, 72.55258319]).reshape((3, 1)),
            decimal=7)

        np.testing.assert_almost_equal(
            colour.models.cie_luv.XYZ_to_Luv(np.array([1.0131677, 1., 2.11217686])),
...
class TestLuv_to_XYZ(unittest.TestCase):
    ...
    def test_Luv_to_XYZ(self):
        ...
        np.testing.assert_almost_equal(
            colour.models.cie_luv.Luv_to_XYZ(np.array([100., -11.27488915, -29.36041662])),
            np.array([0.96907232, 1., 1.12179215]).reshape((3, 1)),
            decimal=7)

        np.testing.assert_almost_equal(
            colour.models.cie_luv.Luv_to_XYZ(np.array([100., 331.44911128, 72.55258319])),
            np.array([1.92001986, 1., -0.1241347]).reshape((3, 1)),
            decimal=7)

        np.testing.assert_almost_equal(
            colour.models.cie_luv.Luv_to_XYZ(np.array([100., -36.17788915, -111.00091702])),
...
class TestLuv_to_LCHuv(unittest.TestCase):
    ...
    def test_Luv_to_LCHuv(self):
        ...
        np.testing.assert_almost_equal(
            colour.models.cie_luv.Luv_to_LCHuv(
                np.array([100., -11.27488915, -29.36041662])),
...
class TestLCHuv_to_Luv(unittest.TestCase):
    ...
    def test_LCHuv_to_Luv(self):
        ...
        np.testing.assert_almost_equal(
            colour.models.cie_luv.LCHuv_to_Luv(
...
class TestLuv_uv_to_xy(unittest.TestCase):
    ...
    def test_Luv_uv_to_xy(self):
        ...
        np.testing.assert_almost_equal(
            colour.models.cie_luv.Luv_uv_to_xy((0.20048615433157738, 0.4654903849082484)),
            (0.31352792378977895, 0.32353408235422665),
            decimal=7)

        np.testing.assert_almost_equal(
            colour.models.cie_luv.Luv_uv_to_xy((0.46412000081619281, 0.54388500014670993)),
            (0.6867305880410077, 0.3576684816384643),
            decimal=7)

        np.testing.assert_almost_equal(
            colour.models.cie_luv.Luv_uv_to_xy((0.18133000048542355, 0.40268999998517152)),
