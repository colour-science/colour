...
class TestXYZ_to_UVW(unittest.TestCase):
    ...
    def test_XYZ_to_UVW(self):
        ...
        np.testing.assert_almost_equal(
            ...
            np.array([-0.90199113, -1.56588889, 8.]).reshape((3, 1)),
            decimal=7)

        np.testing.assert_almost_equal(
            colour.models.cie_uvw.XYZ_to_UVW(
                np.array([1.92001986, 1., - 0.1241347])),
            np.array([26.5159289, 3.8694711, 8.]).reshape((3, 1)),
            decimal=7)

        np.testing.assert_almost_equal(
            colour.models.cie_uvw.XYZ_to_UVW(
                np.array([1.0131677, 1., 2.11217686])),
            np.array([-2.89423113, -5.92004891, 8.]).reshape((3, 1)),
            decimal=7)
