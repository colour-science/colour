...
# Path to the ground-level Prague Sky Model dataset for integration tests.
from colour.phenomena.sky.wilkie2021 import PATH_PRAGUE_SKY_MODEL_DATASET_GROUND

DATASET_PATH = PATH_PRAGUE_SKY_MODEL_DATASET_GROUND
DATASET_AVAILABLE = os.path.isfile(DATASET_PATH)

# Wavelengths used in C++ reference tests.
_WAVELENGTH = np.array([320.0, 380.0, 460.0, 540.0, 620.0, 700.0])

# C++ reference values generated by gen_ref.cpp with the ground-level dataset.
_REF_CASES = {
...
class TestInterpolationParameter:
    """
    Define :func:`colour.phenomena.sky.wilkie2021.linear_interpolation_index_and_factor`
    definition unit tests methods.
    """

    def testlinear_interpolation_index_and_factor(self) -> None:
        """
...
linear_interpolation_index_and_factor` definition.
        ...
        # Single-element break_points (degenerate).
...
class TestLoadDatasetWilkie2021:
    """
    Define :func:`colour.phenomena.sky.wilkie2021.SkyDataset_Wilkie2021`
    definition unit tests methods.
    """
    ...
    def test_SkyDataset_Wilkie2021(self) -> None:
        """
        Test :func:`colour.phenomena.sky.wilkie2021.\
SkyDataset_Wilkie2021` definition.
        """

        ds = SkyDataset_Wilkie2021(DATASET_PATH)

        assert isinstance(ds, SkyDataset_Wilkie2021)
        assert ds.channels == 11
        np.testing.assert_allclose(ds.channel_start, 320.0)
        np.testing.assert_allclose(ds.channel_width, 40.0)
        assert len(ds.visibilities_radiance) >= 1
        assert len(ds.albedos_radiance) >= 1
        assert len(ds.altitudes_radiance) >= 1
        assert len(ds.elevations_radiance) >= 1
        assert len(ds.data_radiance) > 0
        assert len(ds.data_transmittance_u) > 0
        assert len(ds.data_transmittance_v) > 0

    def test_SkyDataset_Wilkie2021_file_not_found(self) -> None:
        """
        Test :func:`colour.phenomena.sky.wilkie2021.\
SkyDataset_Wilkie2021` definition with missing file.
        """
...
compute_sky_parameters_Wilkie2021` definition.
        ...
        for name, case in _REF_CASES.items():
            params = compute_sky_parameters_Wilkie2021(
                np.array(case["view_point"], dtype=float),
                np.array(case["view_direction"], dtype=float),
                case["sun_elevation"],
                case["sun_azimuth"],
                case["visibility"],
                case["albedo"],
            ...
            np.testing.assert_allclose(
                params.theta, ref["theta"], atol=1e-6, err_msg=f"{name}: theta"
            )
            np.testing.assert_allclose(
                params.gamma, ref["gamma"], atol=1e-6, err_msg=f"{name}: gamma"
            )
            np.testing.assert_allclose(
                params.shadow, ref["shadow"], atol=1e-6, err_msg=f"{name}: shadow"
            )
            np.testing.assert_allclose(
                params.zero, ref["zero"], atol=1e-6, err_msg=f"{name}: zero"
            )
            np.testing.assert_allclose(
                params.elevation,
                ref["elevation"],
...
sky_radiance_Wilkie2021` definition.
        ...
        for name, case in _REF_CASES.items():
            ...
            result = sky_radiance_Wilkie2021(ds, params, _WAVELENGTH)
            ref = np.array(case["sky_radiance"])

            np.testing.assert_allclose(
                result, ref, rtol=1e-5, err_msg=f"{name}: sky_radiance"
            )
...
sky_radiance_Wilkie2021` definition with out-of-range wavelengths.
        ...
        # Wavelengths outside dataset range should return zero.
        result = sky_radiance_Wilkie2021(ds, params, np.array([100.0, 5000.0]))
        np.testing.assert_array_equal(result, 0.0)

    def test_sky_radiance_Wilkie2021_n_dimensional(self) -> None:
        """
        Test :func:`colour.phenomena.sky.wilkie2021.\
...
sun_radiance_Wilkie2021` definition.
        ...
        for name, case in _REF_CASES.items():
            ...
            result = sun_radiance_Wilkie2021(ds, params, _WAVELENGTH)
...
sun_radiance_Wilkie2021` definition when looking at the sun.
        ...
        # Look directly at the sun (gamma ~ 0).
        sun_dir = np.array(
            [
        ...
        result = sun_radiance_Wilkie2021(ds, params, _WAVELENGTH)
        # Should be positive for wavelengths in the solar table range.
...
sky_transmittance_Wilkie2021` definition.
        ...
        for name, case in _REF_CASES.items():
            ...
            result = sky_transmittance_Wilkie2021(ds, params, _WAVELENGTH, np.inf)
            ref = np.array(case["transmittance"])

            np.testing.assert_allclose(
                result, ref, rtol=1e-5, err_msg=f"{name}: transmittance"
            )
...
sky_transmittance_Wilkie2021` definition with out-of-range wavelengths.
        ...
        result = sky_transmittance_Wilkie2021(
            ds, params, np.array([100.0, 5000.0]), np.inf
        )
        np.testing.assert_array_equal(result, 0.0)
