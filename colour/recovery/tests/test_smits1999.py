...
class TestMsds_from_RGB_Smits1999:
    ...
    def test_RGB_to_msds_Smits1999(self, xp: ModuleType) -> None:
        ...
        msds = RGB_to_msds_Smits1999(RGB, MSDS_SMITS1999, as_array=True)
...
class TestRGB_to_msds_Smits1999:
    ...
    def test_RGB_to_msds_Smits1999(self, xp: ModuleType) -> None:
        ...
        msds = RGB_to_msds_Smits1999(RGB, as_array=True)
        ...
        # Across the six possible *RGB* channel orderings, exercising every
        # combination of ``argmin`` / ``argmax`` driving the secondary and
        # primary basis selection in *Smits (1999)* §3.
        # ``argmin``, ``argmax`` table:
        # R > G > B  -> primary red,    secondary yellow  (B is min, R is max)
        # R > B > G  -> primary red,    secondary cyan    (G is min, R is max)
        # G > R > B  -> primary green,  secondary yellow  (B is min, G is max)
        # G > B > R  -> primary green,  secondary cyan    (R is min, G is max)
        # B > R > G  -> primary blue,   secondary magenta (G is min, B is max)
        # B > G > R  -> primary blue,   secondary cyan    (R is min, B is max)
        RGBs = xp_as_array(
            [
                [0.7, 0.4, 0.1],  # R > G > B
                [0.7, 0.1, 0.4],  # R > B > G
        ...
        msds = RGB_to_msds_Smits1999(RGBs, as_array=True)

        assert msds.shape == (6, 10)
        # All six recoveries must produce strictly positive reflectances
        # on the visible bands of ``MSDS_SMITS1999``.
        assert bool(xp.all(msds > 0))

        # Tied-channel inputs (white, gray): ``argmin`` / ``argmax``
        # tie-break behaviour maps to the canonical
        # ``MSDS_SMITS1999["white"]`` reflectance for the achromatic cases.
        RGBs = xp_as_array([[1.0, 1.0, 1.0], [0.5, 0.5, 0.5]], xp=xp)
        msds = RGB_to_msds_Smits1999(RGBs, as_array=True)

        white_values = xp_as_array(SDS_SMITS1999["white"].values, xp=xp)
        xp_assert_close(msds[0], white_values, atol=TOLERANCE_ABSOLUTE_TESTS)
        xp_assert_close(msds[1], white_values * 0.5, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestRGB_to_sd_Smits1999:
    ...
    def test_RGB_to_sd_Smits1999(self) -> None:
        ...
        xp_assert_close(
            RGB_to_sd_Smits1999(
                XYZ_to_RGB_Smits1999([0.21781186, 0.12541048, 0.04697113])
            ).values,
            [
                0.07691923,
                0.05870050,
        ...
        xp_assert_close(
            RGB_to_sd_Smits1999(
                XYZ_to_RGB_Smits1999([0.15434689, 0.22960951, 0.09620221])
            ).values,
            [
                0.06981477,
                0.06981351,
        ...
        xp_assert_close(
            RGB_to_sd_Smits1999(
                XYZ_to_RGB_Smits1999([0.07683480, 0.06006092, 0.25833845])
            ).values,
            [
                0.29091152,
                0.29010285,
        ...
        xp_assert_close(
            RGB_to_sd_Smits1999(XYZ_to_RGB_Smits1999([0.0, 1.0, 0.0])).values,
            [
                -0.2549796,
                -0.2848386,
                -0.1634905,
        ...
        xp_assert_close(
            RGB_to_sd_Smits1999(XYZ_to_RGB_Smits1999([1.0, 1.0, 0.0])).values,
            [
                -0.1168428,
                -0.1396982,
                -0.0414535,
        ...
        xp_assert_close(
            RGB_to_sd_Smits1999(XYZ_to_RGB_Smits1999([0.5, 0.0, 1.0])).values,
            [
                1.1938776,
                1.1938776,
                1.1213867,
    ...
    def test_domain_range_scale_RGB_to_sd_Smits1999(self, xp: ModuleType) -> None:
        ...
        RGB_i = xp_as_array(XYZ_to_RGB_Smits1999(XYZ_i), xp=xp)
            ...
            with domain_range_scale(scale):
                xp_assert_close(
                    sd_to_XYZ_integration(
                        RGB_to_sd_Smits1999(RGB_i * xp_as_array(factor_a, xp=xp))
                    ),
                    XYZ_o * factor_b,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
