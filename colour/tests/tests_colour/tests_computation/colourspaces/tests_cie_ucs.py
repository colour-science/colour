...
class TestXYZ_to_UCS(unittest.TestCase):
    ...
    def test_XYZ_to_UCS(self):
        ...
        numpy.testing.assert_almost_equal(
            ...
            numpy.array([7.87055614, 10.34, 12.18252904]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.colourspaces.cie_ucs.XYZ_to_UCS(
                numpy.array([3.08690042, 3.2, 2.68925666])),
            numpy.array([2.05793361, 3.2, 4.60117812]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.colourspaces.cie_ucs.XYZ_to_UCS(
                numpy.array([0.96907232, 1., 1.12179215])),
            numpy.array([0.64604821, 1., 1.57635992]).reshape((3, 1)),
            decimal=7)
    ...
    def test_UCS_to_XYZ(self):
        ...
        numpy.testing.assert_almost_equal(
            colour.computation.colourspaces.cie_ucs.UCS_to_XYZ(
                numpy.array([7.87055614, 10.34, 12.18252904])),
            numpy.array([11.80583421, 10.34, 5.15089229]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.colourspaces.cie_ucs.UCS_to_XYZ(
                numpy.array([2.05793361, 3.2, 4.60117812])),
            numpy.array([3.08690042, 3.2, 2.68925666]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.colourspaces.cie_ucs.UCS_to_XYZ(
                numpy.array([0.64604821, 1., 1.57635992])),
