...
**Description:**
    Defines units tests for :mod:`colour.computation.transformations` module.
...
class TestXYZ_to_xyY(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.XYZ_to_xyY` definition units tests methods.
    """

    def test_XYZ_to_xyY(self):
        """
        Tests :func:`colour.computation.transformations.XYZ_to_xyY` definition.
        """

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.XYZ_to_xyY(
                numpy.matrix([11.80583421, 10.34, 5.15089229]).reshape((3, 1))),
            numpy.matrix([0.4325, 0.3788, 10.34]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.XYZ_to_xyY(numpy.matrix([3.08690042, 3.2, 2.68925666]).reshape((3, 1))),
            numpy.matrix([0.3439, 0.3565, 3.20]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.XYZ_to_xyY(numpy.matrix([0, 0, 0]).reshape((3, 1)), (0.34567, 0.35850)),
...
class TestxyY_to_XYZ(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.xyY_to_XYZ` definition units tests methods.
    """

    def test_xyY_to_XYZ(self):
        """
        Tests :func:`colour.computation.transformations.xyY_to_XYZ` definition.
        """

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.xyY_to_XYZ(numpy.matrix([0.4325, 0.3788, 10.34]).reshape((3, 1))),
        ...
        numpy.testing.assert_almost_equal(
            colour.computation.transformations.xyY_to_XYZ(numpy.matrix([0.3439, 0.3565, 3.20]).reshape((3, 1))),
        ...
        numpy.testing.assert_almost_equal(
            colour.computation.transformations.xyY_to_XYZ(numpy.matrix([0.4325, 0., 10.34]).reshape((3, 1))),
            numpy.matrix([0.,
                          0.,
                          0.]).reshape((3, 1)),
            decimal=7)
...
class Testxy_to_XYZ(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.xy_to_XYZ` definition units tests methods.
    """

    def test_xy_to_XYZ(self):
        """
        Tests :func:`colour.computation.transformations.xy_to_XYZ` definition.
        """

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.xy_to_XYZ((0.32207410281368043,
                                                          0.3315655001362353)),
        ...
        numpy.testing.assert_almost_equal(
            colour.computation.transformations.xy_to_XYZ((0.32174206617150575,
                                                          0.337609723160027)),
            numpy.matrix([0.953,
                          1.000,
                          1.009]).reshape((3, 1)),
        ...
        numpy.testing.assert_almost_equal(
            colour.computation.transformations.xy_to_XYZ((0.4474327628361859,
                                                          0.4074979625101875)),
            numpy.matrix([1.098,
                          1.000,
                          0.356]).reshape((3, 1)),
...
class TestXYZ_to_xy(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.XYZ_to_xy` definition units tests methods.
    """

    def test_XYZ_to_xy(self):
        """
        Tests :func:`colour.computation.transformations.XYZ_to_xy` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_xy((0.97137399,
                                                                                        1.,
                                                                                        1.04462134)),
                                          (0.32207410281368043, 0.3315655001362353),
                                          decimal=7)

        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_xy((0.953,
                                                                                        1.000,
                                                                                        1.009)),
                                          (0.32174206617150575, 0.337609723160027),
                                          decimal=7)

        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_xy((1.098,
                                                                                        1.000,
                                                                                        0.356)),
...
class TestXYZ_to_RGB(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.XYZ_to_RGB` definition units tests methods.
    """

    def test_XYZ_to_RGB(self):
        """
        Tests :func:`colour.computation.transformations.XYZ_to_RGB` definition.
        """

        for xyY, XYZ, RGB in sRGB_LINEAR_COLORCHECKER_2005:
            numpy.testing.assert_almost_equal(
                colour.computation.transformations.XYZ_to_RGB(numpy.matrix(XYZ).reshape((3, 1)),
                                                              (0.34567, 0.35850),
                                                              (0.31271, 0.32902),
                                                              "Bradford",
                                                              numpy.matrix(
...
class TestRGB_to_XYZ(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.RGB_to_XYZ` definition units tests methods.
    """

    def test_RGB_to_XYZ(self):
        """
        Tests :func:`colour.computation.transformations.RGB_to_XYZ` definition.
        """

        for xyY, XYZ, RGB in sRGB_LINEAR_COLORCHECKER_2005:
            numpy.testing.assert_almost_equal(colour.computation.transformations.RGB_to_XYZ(RGB,
                                                                                            (0.31271, 0.32902),
                                                                                            (0.34567, 0.35850),
                                                                                            "Bradford",
                                                                                            numpy.matrix(
                                                                                                [0.41238656, 0.35759149,
                                                                                                 0.18045049,
                                                                                                 0.21263682, 0.71518298,
                                                                                                 0.0721802,
...
class TestxyY_to_RGB(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.xyY_to_RGB` definition units tests methods.
    """

    def test_xyY_to_RGB(self):
        """
        Tests :func:`colour.computation.transformations.xyY_to_RGB` definition.
        """

        for xyY, XYZ, RGB in sRGB_LINEAR_COLORCHECKER_2005:
            numpy.testing.assert_almost_equal(
                colour.computation.transformations.xyY_to_RGB(numpy.matrix(xyY).reshape((3, 1)),
                                                              (0.34567, 0.35850),
                                                              (0.31271, 0.32902),
                                                              "Bradford",
                                                              numpy.matrix(
...
class TestRGB_to_xyY(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.RGB_to_xyY` definition units tests methods.
    """

    def test_RGB_to_xyY(self):
        """
        Tests :func:`colour.computation.transformations.RGB_to_xyY` definition.
        """

        for xyY, XYZ, RGB in sRGB_LINEAR_COLORCHECKER_2005:
            numpy.testing.assert_almost_equal(colour.computation.transformations.RGB_to_xyY(RGB,
                                                                                            (0.31271, 0.32902),
                                                                                            (0.34567, 0.35850),
                                                                                            "Bradford",
                                                                                            numpy.matrix(
...
class TestXYZ_to_UCS(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.XYZ_to_UCS` definition units tests methods.
    """

    def test_XYZ_to_UCS(self):
        """
        Tests :func:`colour.computation.transformations.XYZ_to_UCS` definition.
        """
        ...
        numpy.testing.assert_almost_equal(
            colour.computation.transformations.XYZ_to_UCS(numpy.matrix([3.08690042, 3.2, 2.68925666]).reshape((3, 1))),
            numpy.matrix([2.05793361, 3.2, 4.60117812]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.XYZ_to_UCS(numpy.matrix([0.96907232, 1., 1.12179215]).reshape((3, 1))),
...
class TestUCS_to_XYZ(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.UCS_to_XYZ` definition units tests methods.
    """

    def test_UCS_to_XYZ(self):
        """
        Tests :func:`colour.computation.transformations.UCS_to_XYZ` definition.
        """

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.UCS_to_XYZ(
                numpy.matrix([7.87055614, 10.34, 12.18252904]).reshape((3, 1))),
            numpy.matrix([11.80583421, 10.34, 5.15089229]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.UCS_to_XYZ(numpy.matrix([2.05793361, 3.2, 4.60117812]).reshape((3, 1))),
            numpy.matrix([3.08690042, 3.2, 2.68925666]).reshape((3, 1)),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.UCS_to_XYZ(numpy.matrix([0.64604821, 1., 1.57635992]).reshape((3, 1))),
...
class TestUCS_to_uv(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.UCS_to_uv` definition units tests methods.
    """

    def test_UCS_to_uv(self):
        """
        Tests :func:`colour.computation.transformations.UCS_to_uv` definition.
        """

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.UCS_to_uv(
                numpy.matrix([7.87055614, 10.34, 12.18252904]).reshape((3, 1))),
            (0.25895877609618834, 0.34020896328103534),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.UCS_to_uv(numpy.matrix([2.05793361, 3.2, 4.60117812]).reshape((3, 1))),
            (0.20873418076173886, 0.32457285074301517),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.UCS_to_uv(numpy.matrix([0.64604821, 1., 1.57635992]).reshape((3, 1))),
...
class TestUCS_uv_to_xy(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.UCS_uv_to_xy` definition units tests methods.
    """

    def test_UCS_uv_to_xy(self):
        """
        Tests :func:`colour.computation.transformations.UCS_uv_to_xy` definition.
        """

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.UCS_uv_to_xy((0.2033733344733139, 0.3140500001549052)),
            (0.32207410281368043, 0.33156550013623537),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.UCS_uv_to_xy((0.20873418102926322, 0.32457285063327812)),
            (0.3439000000209443, 0.35650000010917804),
            decimal=7)

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.UCS_uv_to_xy((0.25585459629500179, 0.34952813701502972)),
...
class TestXYZ_to_UVW(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.XYZ_to_UVW` definition units tests methods.
    """

    def test_XYZ_to_UVW(self):
        """
        Tests :func:`colour.computation.transformations.XYZ_to_UVW` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_UVW(numpy.matrix([0.96907232,
                                                                                                      1.,
                                                                                                      1.12179215]).reshape(
            (3, 1))),
        ...
        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_UVW(numpy.matrix([1.92001986,
                                                                                                      1.,
                                                                                                      -0.1241347]).reshape(
            (3, 1))),
        ...
        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_UVW(numpy.matrix([1.0131677,
                                                                                                      1.,
                                                                                                      2.11217686]).reshape(
            (3, 1))),
...
class TestXYZ_to_Luv(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.XYZ_to_Luv` definition units tests methods.
    """

    def test_XYZ_to_Luv(self):
        """
        Tests :func:`colour.computation.transformations.XYZ_to_Luv` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_Luv(numpy.matrix([0.96907232,
                                                                                                      1.,
                                                                                                      1.12179215]).reshape(
            (3, 1))),
        ...
        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_Luv(numpy.matrix([1.92001986,
                                                                                                      1.,
                                                                                                      -0.1241347]).reshape(
            (3, 1))),
        ...
        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_Luv(numpy.matrix([1.0131677,
                                                                                                      1.,
                                                                                                      2.11217686]).reshape(
            (3, 1))),
...
class TestLuv_to_XYZ(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.Luv_to_XYZ` definition units tests methods.
    """

    def test_Luv_to_XYZ(self):
        """
        Tests :func:`colour.computation.transformations.Luv_to_XYZ` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.Luv_to_XYZ(numpy.matrix([100.,
                                                                                                      -11.27488915,
                                                                                                      -29.36041662]).reshape(
            (3, 1))),
                                          numpy.matrix([0.96907232, 1., 1.12179215]).reshape((3, 1)),
...
class TestLuv_to_uv(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.Luv_to_uv` definition units tests methods.
    """

    def test_Luv_to_uv(self):
        """
        Tests :func:`colour.computation.transformations.Luv_to_uv` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.Luv_to_uv(numpy.matrix([100.,
                                                                                                     -11.27488915,
                                                                                                     -29.36041662]).reshape(
            (3, 1))),
...
class TestLuv_to_LCHuv(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.Luv_to_LCHuv` definition units tests methods.
    """

    def test_Luv_to_LCHuv(self):
        """
        Tests :func:`colour.computation.transformations.Luv_to_LCHuv` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.Luv_to_LCHuv(numpy.matrix([100.,
                                                                                                        -11.27488915,
                                                                                                        -29.36041662]).reshape(
            (3, 1))),
...
class TestLCHuv_to_Luv(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.LCHuv_to_Luv` definition units tests methods.
    """

    def test_LCHuv_to_Luv(self):
        """
        Tests :func:`colour.computation.transformations.LCHuv_to_Luv` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.LCHuv_to_Luv(numpy.matrix([100.,
                                                                                                        31.45086945,
...
class TestLuv_uv_to_xy(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.Luv_uv_to_xy` definition units tests methods.
    """

    def test_Luv_uv_to_xy(self):
        """
        Tests :func:`colour.computation.transformations.Luv_uv_to_xy` definition.
        """

        numpy.testing.assert_almost_equal(
            colour.computation.transformations.Luv_uv_to_xy((0.20048615433157738, 0.4654903849082484)),
            (0.31352792378977895, 0.32353408235422665),
            decimal=7)

        numpy.testing.assert_almost_equal(colour.computation.transformations.Luv_uv_to_xy((0.46412000081619281,
                                                                                           0.54388500014670993)),
                                          (0.6867305880410077, 0.3576684816384643),
                                          decimal=7)

        numpy.testing.assert_almost_equal(colour.computation.transformations.Luv_uv_to_xy((0.18133000048542355,
                                                                                           0.40268999998517152)),
...
class TestXYZ_to_Lab(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.XYZ_to_Lab` definition units tests methods.
    """

    def test_XYZ_to_Lab(self):
        """
        Tests :func:`colour.computation.transformations.XYZ_to_Lab` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_Lab(numpy.matrix([0.96907232,
                                                                                                      1.,
                                                                                                      1.12179215]).reshape(
            (3, 1))),
        ...
        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_Lab(numpy.matrix([1.92001986,
                                                                                                      1.,
                                                                                                      -0.1241347]).reshape(
            (3, 1))),
        ...
        numpy.testing.assert_almost_equal(colour.computation.transformations.XYZ_to_Lab(numpy.matrix([1.0131677,
                                                                                                      1.,
                                                                                                      2.11217686]).reshape(
            (3, 1))),
...
class TestLab_to_XYZ(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.Lab_to_XYZ` definition units tests methods.
    """

    def test_Lab_to_XYZ(self):
        """
        Tests :func:`colour.computation.transformations.Lab_to_XYZ` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.Lab_to_XYZ(numpy.matrix([100.,
                                                                                                      0.83871284,
                                                                                                      -21.55579303]).reshape(
            (3, 1))),
                                          numpy.matrix([0.96907232, 1., 1.12179215]).reshape((3, 1)),
...
class TestLab_to_LCHab(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.Lab_to_LCHab` definition units tests methods.
    """

    def test_Lab_to_LCHab(self):
        """
        Tests :func:`colour.computation.transformations.Lab_to_LCHab` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.Lab_to_LCHab(numpy.matrix([100.,
                                                                                                        0.83871284,
                                                                                                        -21.55579303]).reshape(
            (3, 1))),
...
class TestLCHab_to_Lab(unittest.TestCase):
    """
    Defines :func:`colour.computation.transformations.LCHab_to_Lab` definition units tests methods.
    """

    def test_LCHab_to_Lab(self):
        """
        Tests :func:`colour.computation.transformations.LCHab_to_Lab` definition.
        """

        numpy.testing.assert_almost_equal(colour.computation.transformations.LCHab_to_Lab(numpy.matrix([100.,
                                                                                                        21.57210357,
                                                                                                        272.2281935]).reshape(
            (3, 1))),
                                          numpy.matrix([100.,
