...
**Description:**
    Defines units tests for :mod:`colour.computation.tristimulus` module.
...
RELATIVE_SPD_DATA = colour.computation.spectrum.SpectralPowerDistribution("Custom", {
    340: 0.0000,
    345: 0.0000,
    350: 0.0000,
    355: 0.0000,
...
class TestSpectral_to_XYZ(unittest.TestCase):
    """
    Defines :func:`colour.computation.tristimulus.spectral_to_XYZ` definition units tests methods.
    """

    def test_spectral_to_XYZ(self):
        """
        Tests :func:`colour.computation.tristimulus.spectral_to_XYZ` definition.
        """

        cmfs = colour.dataset.cmfs.CMFS.get("CIE 1931 2 Degree Standard Observer")
        numpy.testing.assert_almost_equal(
            colour.computation.tristimulus.spectral_to_XYZ(RELATIVE_SPD_DATA.zeros(*cmfs.shape),
                                                           cmfs,
                                                           colour.dataset.illuminants.spds.ILLUMINANTS_RELATIVE_SPDS.get(
...
class TestWavelength_to_XYZ(unittest.TestCase):
    """
    Defines :func:`colour.computation.tristimulus.wavelength_to_XYZ` definition units tests methods.
    """

    def test_wavelength_to_XYZ(self):
        """
        Tests :func:`colour.computation.tristimulus.wavelength_to_XYZ` definition.
        """

        numpy.testing.assert_almost_equal(
            colour.computation.tristimulus.wavelength_to_XYZ(480,
                                                             colour.dataset.cmfs.CMFS.get(
        ...
        numpy.testing.assert_almost_equal(
            colour.computation.tristimulus.wavelength_to_XYZ(641.5,
                                                             colour.dataset.cmfs.CMFS.get(
                                                                 "CIE 2012 2 Degree Standard Observer")),
